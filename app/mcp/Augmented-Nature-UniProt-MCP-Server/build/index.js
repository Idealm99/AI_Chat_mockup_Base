...
const isValidProteinInfoArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
const isValidCompareProteinsArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
        Array.isArray(args.accessions) &&
        args.accessions.length > 1 &&
        args.accessions.length <= 10 &&
        args.accessions.every((acc) => typeof acc === 'string' && acc.length > 0) &&
...
const isValidBatchLookupArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
        Array.isArray(args.accessions) &&
...
const isValidAdvancedSearchArgs = (args) => {
    return (typeof args === 'object' &&
        ...
        (args.keywords === undefined || (Array.isArray(args.keywords) && args.keywords.every((k) => typeof k === 'string'))) &&
        (args.size === undefined || (typeof args.size === 'number' && args.size > 0 && args.size <= 500)));
};
const isValidFunctionSearchArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
const isValidTaxonomySearchArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
const isValidAccessionValidateArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
        typeof args.accession === 'string' &&
        args.accession.length > 0);
};
const isValidLocalizationSearchArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
class UniProtServer {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            if (proteinMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch protein ${accession}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
            ...
            if (sequenceMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch sequence for ${accession}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
    ...
    async handleGetProteinSequence(args) {
    ...
    async handleGetProteinFeatures(args) {
        if (!isValidProteinInfoArgs(args)) {
        ...
        try {
            ...
            const features = {
                ...
                domains: protein.features?.filter((f) => f.type === 'Domain') || [],
                activeSites: protein.features?.filter((f) => f.type === 'Active site') || [],
                bindingSites: protein.features?.filter((f) => f.type === 'Binding site') || [],
            };
            return {
                content: [
                    {
    ...
    // Comparative & Evolutionary Analysis Tools
    async handleCompareProteins(args) {
        ...
        try {
            ...
            for (const accession of args.accessions) {
                ...
                comparisons.push({
                    ...
                    domains: protein.features?.filter((f) => f.type === 'Domain').length || 0,
                });
            }
            return {
                content: [
    ...
    async handleGetProteinHomologs(args) {
    ...
    async handleGetProteinOrthologs(args) {
        if (!isValidHomologArgs(args)) {
    ...
    async handleGetPhylogeneticInfo(args) {
        if (!isValidProteinInfoArgs(args)) {
        ...
        try {
            ...
            const phylogeneticInfo = {
                ...
                evolutionaryOrigin: protein.comments?.filter((c) => c.commentType === 'EVOLUTIONARY ORIGIN') || [],
                phylogeneticRange: protein.comments?.filter((c) => c.commentType === 'PHYLOGENETIC RANGE') || [],
            };
            return {
                content: [
                    {
    ...
    // Structure & Function Analysis Tools
    async handleGetProteinStructure(args) {
        if (!isValidProteinInfoArgs(args)) {
        ...
        try {
            ...
            const structureInfo = {
                accession: protein.primaryAccession,
                pdbReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'PDB') || [],
                structuralFeatures: protein.features?.filter((f) => ['Secondary structure', 'Turn', 'Helix', 'Beta strand'].includes(f.type)) || [],
                structuralComments: protein.comments?.filter((c) => c.commentType === 'SUBUNIT') || [],
            };
            return {
                content: [
                    {
    ...
    async handleGetProteinDomainsDetailed(args) {
        if (!isValidProteinInfoArgs(args)) {
        ...
        try {
            ...
            const domainInfo = {
                accession: protein.primaryAccession,
                domains: protein.features?.filter((f) => f.type === 'Domain') || [],
                regions: protein.features?.filter((f) => f.type === 'Region') || [],
                repeats: protein.features?.filter((f) => f.type === 'Repeat') || [],
                interproReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'InterPro') || [],
                pfamReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'Pfam') || [],
                smartReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'SMART') || [],
            };
            return {
                content: [
                    {
    ...
    async handleGetProteinVariants(args) {
        if (!isValidProteinInfoArgs(args)) {
        ...
        try {
            ...
            const variantInfo = {
                accession: protein.primaryAccession,
                naturalVariants: protein.features?.filter((f) => f.type === 'Natural variant') || [],
                mutagenesisFeatures: protein.features?.filter((f) => f.type === 'Mutagenesis') || [],
                diseaseVariants: protein.features?.filter((f) => f.type === 'Natural variant' && f.association?.disease) || [],
                polymorphisms: protein.comments?.filter((c) => c.commentType === 'POLYMORPHISM') || [],
            };
            return {
                content: [
                    {
    ...
    async handleAnalyzeSequenceComposition(args) {
        if (!isValidProteinInfoArgs(args)) {
    ...
    // Biological Context Tools
    async handleGetProteinPathways(args) {
        if (!isValidProteinInfoArgs(args)) {
        ...
        try {
            ...
            const pathwayInfo = {
                accession: protein.primaryAccession,
                keggReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'KEGG') || [],
                reactomeReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'Reactome') || [],
                pathwayComments: protein.comments?.filter((c) => c.commentType === 'PATHWAY') || [],
                biologicalProcess: protein.comments?.filter((c) => c.commentType === 'FUNCTION') || [],
            };
            return {
                content: [
                    {
    ...
    async handleGetProteinInteractions(args) {
        ...
        try {
            ...
            const interactionInfo = {
                accession: protein.primaryAccession,
                stringReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'STRING') || [],
                intactReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'IntAct') || [],
                interactionComments: protein.comments?.filter((c) => c.commentType === 'INTERACTION') || [],
                subunitComments: protein.comments?.filter((c) => c.commentType === 'SUBUNIT') || [],
            };
            return {
                content: [
                    {
    ...
    async handleSearchByFunction(args) {
    ...
    async handleSearchByLocalization(args) {
    ...
    async handleBatchProteinLookup(args) {
        ...
        try {
            ...
            for (let i = 0; i < args.accessions.length; i += chunkSize) {
                ...
                const chunkResults = await Promise.all(chunk.map(async (accession) => {
                    try {
                        const response = await this.apiClient.get(`/uniprotkb/${accession}`, {
    ...
    async handleSearchByTaxonomy(args) {
    ...
    async handleGetExternalReferences(args) {
        ...
        try {
            ...
            const externalRefs = {
                ...
                pdbReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'PDB') || [],
                emblReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'EMBL') || [],
                refseqReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'RefSeq') || [],
                ensemblReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'Ensembl') || [],
                goReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'GO') || [],
            };
            return {
                content: [
                    {
    ...
    async handleGetLiteratureReferences(args) {
        ...
        try {
            ...
            const literatureInfo = {
                ...
                pubmedReferences: protein.uniProtKBCrossReferences?.filter((ref) => ref.database === 'PubMed') || [],
    ...
    async handleGetAnnotationConfidence(args) {
        if (!isValidProteinInfoArgs(args)) {
        ...
        try {
            ...
            const confidenceInfo = {
                ...
                evidenceCodes: protein.features?.map((f) => f.evidences).flat().filter(Boolean) || [],
    ...
    // Export & Utility Tools
    async handleExportProteinData(args) {
        if (!isValidProteinInfoArgs(args)) {
    ...
    async handleValidateAccession(args) {
