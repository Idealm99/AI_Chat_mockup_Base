...
const isValidGeneAnnotationArgs = (args) => typeof args === 'object' &&
    args !== null &&
    typeof args.gene_id === 'string' &&
    args.gene_id.trim().length > 0;
const isValidGeneQueryArgs = (args) => typeof args === 'object' &&
    args !== null &&
    typeof args.q === 'string' &&
    args.q.trim().length > 0;
const isValidVariantAnnotationArgs = (args) => typeof args === 'object' &&
    args !== null &&
    typeof args.variant_id === 'string' &&
    args.variant_id.trim().length > 0;
const isValidVariantQueryArgs = (args) => typeof args === 'object' &&
    args !== null &&
    typeof args.q === 'string' &&
    args.q.trim().length > 0;
const isValidBatchGeneArgs = (args) => typeof args === 'object' &&
    args !== null &&
    Array.isArray(args.gene_ids) &&
    args.gene_ids.length > 0 &&
    args.gene_ids.length <= 1000 &&
    args.gene_ids.every((id) => typeof id === 'string');
const isValidBatchVariantArgs = (args) => typeof args === 'object' &&
    args !== null &&
    Array.isArray(args.variant_ids) &&
    args.variant_ids.length > 0 &&
    args.variant_ids.length <= 1000 &&
    args.variant_ids.every((id) => typeof id === 'string');
class BioThingsServer {
    ...
    async handleGeneAnnotation(args) {
        if (!isValidGeneAnnotationArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene annotation arguments. gene_id is required.');
        }
        const params = {};
        ...
        const response = await this.axiosInstance.get(`${MYGENE_BASE_URL}/gene/${encodeURIComponent(args.gene_id)}`, { params });
        return {
            content: [{
                    type: 'text',
    ...
    async handleGeneQuery(args) {
        if (!isValidGeneQueryArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid gene query arguments. q is required.');
        }
    ...
    async handleVariantAnnotation(args) {
        if (!isValidVariantAnnotationArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid variant annotation arguments. variant_id is required.');
        }
        const params = {};
        if (args.fields && args.fields !== 'all') {
            params.fields = args.fields;
        }
        const response = await this.axiosInstance.get(`${MYVARIANT_BASE_URL}/variant/${encodeURIComponent(args.variant_id)}`, { params });
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify(response.data, null, 2)
    ...
    async handleVariantQuery(args) {
        if (!isValidVariantQueryArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid variant query arguments. q is required.');
        }
        const params = { q: args.q };
        if (args.size)
            params.size = args.size;
    ...
    async handleBatchGeneQuery(args) {
        if (!isValidBatchGeneArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid batch gene arguments. gene_ids array is required (max 1000 items).');
        }
        const data = new URLSearchParams();
        data.append('ids', args.gene_ids.join(','));
        if (args.fields)
            data.append('fields', args.fields);
        if (args.species)
            data.append('species', args.species);
        const response = await this.axiosInstance.post(`${MYGENE_BASE_URL}/gene`, data, {
            headers: {
    ...
    async handleBatchVariantQuery(args) {
        if (!isValidBatchVariantArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Invalid batch variant arguments. variant_ids array is required (max 1000 items).');
        }
        const data = new URLSearchParams();
        data.append('ids', args.variant_ids.join(','));
        if (args.fields)
            data.append('fields', args.fields);
        const response = await this.axiosInstance.post(`${MYVARIANT_BASE_URL}/variant`, data, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
        });
    ...
    async handleSearchGenesByPathway(args) {
        if (!args || typeof args.pathway_name !== 'string') {
            throw new McpError(ErrorCode.InvalidParams, 'pathway_name is required');
        }
    ...
    async handleSearchGenesByGoTerm(args) {
        if (!args || typeof args.go_term !== 'string') {
            throw new McpError(ErrorCode.InvalidParams, 'go_term is required');
        }
    ...
    async handleSearchVariantsByGene(args) {
        if (!args || typeof args.gene_symbol !== 'string') {
            throw new McpError(ErrorCode.InvalidParams, 'gene_symbol is required');
        }
        ...
        if (variantTypes.length > 0 && !variantTypes.includes('all')) {
            const typeQuery = variantTypes.map((type) => `dbsnp.vartype:${type}`).join(' OR ');
            query += ` AND (${typeQuery})`;
        }
    ...
    async handleSearchPathogenicVariants(args) {
        const size = args?.size || 100;
        ...
        if (args?.gene_list && Array.isArray(args.gene_list) && args.gene_list.length > 0) {
            const geneQuery = args.gene_list.map((gene) => `dbnsfp.genename:"${gene}"`).join(' OR ');
            query += ` AND (${geneQuery})`;
        }
        // Add disease terms if provided
        if (args?.disease_terms && Array.isArray(args.disease_terms) && args.disease_terms.length > 0) {
            const diseaseQuery = args.disease_terms.map((term) => `clinvar.conditions.name:"${term}"`).join(' OR ');
    ...
    async handleGetGeneOrthologs(args) {
        ...
        const geneResponse = await this.axiosInstance.get(`${MYGENE_BASE_URL}/gene/${encodeURIComponent(args.gene_id)}`, { params: { fields: 'homologene' } });
        ...
        const speciesQuery = targetSpecies.map((species) => `taxid:"${species}"`).join(' OR ');
    ...
    async handleSearchDrugTargetGenes(args) {
        if (!args || typeof args.drug_name !== 'string') {
            throw new McpError(ErrorCode.InvalidParams, 'drug_name is required');
        }
    ...
    async handleGetGenomicIntervalGenes(args) {
        if (!args || typeof args.chromosome !== 'string' || typeof args.start !== 'number' || typeof args.end !== 'number') {
            throw new McpError(ErrorCode.InvalidParams, 'chromosome, start, and end are required');
        }
        const species = args.species || 'human';
        ...
        if (!geneTypes.includes('all')) {
            const typeQuery = geneTypes.map((type) => `type_of_gene:"${type}"`).join(' OR ');
    ...
    async handleSearchVariantsByPopulationFrequency(args) {
